...
function buildOverview({assets, portfolios, liabilities, currency = null, latestSnapshot = null}) {
  ...
  const currencies = Array.from(new Set(
    [...activeAssets, ...activePortfolios, ...activeLiabilities].map((row) => normalizeCurrency(row.currency, 'CAD')),
  )).sort()
  ...
  for (const liability of activeLiabilities) {
    ...
    const amount = valueWithOwnership(liability.currentBalance, 100)
    const totals = bucket(code)
...
async function getWealthOverview(prisma, options = {}) {
  ...
  const [assets, portfolios, liabilities, latestSnapshot] = await Promise.all([
    prisma.asset.findMany({orderBy: [{status: 'asc'}, {name: 'asc'}]}),
    prisma.portfolio.findMany({orderBy: [{status: 'asc'}, {name: 'asc'}], include: {account: true}}),
    prisma.liability.findMany({orderBy: [{status: 'asc'}, {name: 'asc'}], include: {account: true, securedAsset: true}}),

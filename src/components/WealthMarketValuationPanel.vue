...
function unwrapIpc<T>(response: IpcResponse<T> | null | undefined): {
  data: T | null;
  error: MarketDataErrorLite | null
} {
  if (!response) return {data: null, error: {code: 'NO_RESPONSE', message: 'Aucune réponse IPC.'}}
...
async function loadMarketValuations() {
  ...
  try {
    ...
    for (const asset of activeRecords(props.assets) as WealthAssetWithMarketInstrument[]) {
      ...
      const snapshotResult = instrumentId ? await loadLatestSnapshot(marketDataApi, instrumentId) : {
        snapshot: null,
        error: null
      }
    ...
    rows.value = [
      ...activeRecords(props.assets).map((asset) => createWealthAssetValuationRow({
        asset,
        summaryCurrency: props.summaryCurrency
      })),
...
async function loadLatestSnapshot(marketDataApi: MarketDataRendererApi | null, instrumentId: number) {
  if (!marketDataApi?.getLatestSnapshot) {
    return {snapshot: null, error: {code: 'PROVIDER_UNAVAILABLE', message: 'API market data indisponible.'}}
  }

  try {
    const response = unwrapIpc(await marketDataApi.getLatestSnapshot({instrumentId}))
    return {snapshot: response.data, error: response.error}
  } catch (error) {
    return {
      snapshot: null,
      error: {
...
<template>
  <article class="rounded-[2rem] border border-slate-800 bg-slate-950 p-5 shadow-sm">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
      <div>
        ...
        <p class="mt-1 max-w-3xl text-sm leading-6 text-slate-400">
          ...
          valeur
          manuelle et continue de contribuer à la valeur nette.
        </p>
      </div>
      <button
          type="button"
    ...
    <div class="mt-5 grid gap-3 md:grid-cols-4">
      ...
      <div class="rounded-2xl border border-red-900/60 bg-red-950/30 p-4">
        <p class="text-xs font-semibold uppercase tracking-[0.18em] text-red-300">Provider</p>
        <p class="mt-2 text-2xl font-semibold text-white">{{ summary.totalErrorCount }}</p>
        <p class="mt-1 text-xs text-red-100/70">Erreur sans blocage écran</p>
      </div>
    </div>
